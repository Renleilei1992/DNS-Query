...
static const char* AliyunDNS = "223.5.5.5";
static const char* MobileDNS = "114.114.114.114";
static const char* GoogleDNS = "8.8.8.8";
...
int main() {
	...
	//_sleep(2000);
	Sleep(2000);
	task.GetHostByName(queryUrl, AliyunDNS, false);
	Sleep(2000);
	task.GetHostByName(queryUrl, GoogleDNS, false);
	Sleep(2000);
	task.GetHostByName(queryUrl, MobileDNS, false);
	Sleep(2000);
...
// 此函数调用了windows的 api: DnsQuery_A [https://docs.microsoft.com/en-us/windows/desktop/api/windns/nf-windns-dnsquery_a]
void dnsQuery()
{
	PIP4_ARRAY pSrvList = NULL;									// pinter to IP4_ARRAY structure
	pSrvList = (PIP4_ARRAY)LocalAlloc(LPTR, sizeof(IP4_ARRAY));	// Allocate memory for IP4_ARRAY structure
	if (!pSrvList) {
		printf("Memory allocation failed \n");
		exit(1);
	}

	const char* queryDns = AliyunDNS;
	pSrvList->AddrCount = 1;
	pSrvList->AddrArray[0] = inet_addr(queryDns); //DNS server IP address

	printf("--------------------------dnsQuery()--------------------------dnsServer: [%s] \n", queryDns);
