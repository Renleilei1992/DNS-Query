...
BOOL RequestDNS::DNSLookup(ULONG ulDNSServerIP, char * szDomainName, std::vector<std::string> *pvecstrIPList, std::vector<ULONG>* pveculIPList, std::vector<std::string>* pvecstrCNameList, ULONG ulTimeout, ULONG * pulTimeSpent)
{
	...
	//std::vector<ULONG> *pveculIPList_tmp = NULL;
	//if (pvecstrIPList != NULL)
	//{
	//	std::vector<ULONG> veculIPList;
	//	pveculIPList_tmp = &veculIPList;
	//}

	// 配置SOCKET
	sockaddr_in sockAddrDNSServer;
	sockAddrDNSServer.sin_family = AF_INET;
	//sockAddrDNSServer.sin_addr.s_addr = inet_addr("223.5.5.5");
...
BOOL RequestDNS::Init()
{
	...
	WSAEventSelect(m_socket, m_event, FD_READ);
...
*/
BOOL RequestDNS::EncodeDotStr(char *szDotStr, char *szEncodedStr, USHORT nEncodedStrSize)
{
...
*/
BOOL RequestDNS::DecodeDotStr(char *szEncodedStr, USHORT *pusEncodedStrLen, char *szDotStr, USHORT nDotStrSize, char *szPacketStartPos)
{
...
BOOL RequestDNS::SendDNSRequest(sockaddr_in sockAddrDNSServer, char *szDomainName)
{
	//printf("%s dns:[%s], domain: [%s] \n", __FUNCTION__, sockAddrDNSServer.sin_addr, szDomainName);
	...
	pDNSHeader->usTransID = (USHORT)GetCurrentProcessId();	// 本进程ID
	...
	if (sendto(m_socket, m_szDNSPacket, nDNSPacketSize, 0, (sockaddr*)&sockAddrDNSServer, sizeof(sockAddrDNSServer)) == SOCKET_ERROR)
	{
		return FALSE;
	}
...
BOOL RequestDNS::RecvDNSResponse(sockaddr_in sockAddrDNSServer, ULONG ulTimeout, std::vector<ULONG> *pveculIPList, std::vector<std::string> *pvecstrCNameList, ULONG *pulTimeSpent)
{
	...
	while (TRUE)
	{
		if (WSAWaitForMultipleEvents(1, &m_event, FALSE, 100, FALSE) != WSA_WAIT_TIMEOUT)
		{
			...
			WSAEnumNetworkEvents(m_socket, m_event, &netEvent);
			...
			if (netEvent.lNetworkEvents & FD_READ)
			{
				...
				//接收响应报文
				if (recvfrom(m_socket, recvbuf, 1024, 0, (struct sockaddr*)&sockAddrDNSServer, &nSockaddrDestSize) != SOCKET_ERROR)
				{
...
ULONG RequestDNS::GetTickCountCalibrate()
{
